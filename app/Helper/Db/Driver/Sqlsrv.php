...
// +----------------------------------------------------------------------
namespace Helper\Db\Driver;
use Exception\FrameworkException;
...
class Sqlsrv extends Db{
    ...
    public function __construct($config='') {
        if ( !function_exists('sqlsrv_connect') ) {
            ...
            $code = ErrorCode::ERRPR_FRAMEWORK_NO_SUPPORT_DRIVER;
          	$message = ErrorCode::getMessage($code);
            throw new FrameworkException($message, $code);
        }
        if(!empty($config)) {
            $this->config	=	$config;
        }
    ...
    public function error($result = true) {
        ...
        if ($result){
        	...
        	throw new FrameworkException($this->error, 500);
        }
        return $this->error;
    }
}